...
A: Yes, on **embedded dashboards** you can set a filter to _Hidden with Preset_ — the value is applied silently without showing the control to the viewer. See [Embedded Dashboard Filters](/add-filters-and-environments-in-embedded-dashboard-programatically) for details.

**Q: How do I scope an embedded dashboard to a specific customer or tenant?**\
A: Use the hidden-preset filter mode on embeds, or pass filter values as a URL parameter (`?filter=<base64-json>`). See [Embedded Dashboard Filters](/add-filters-and-environments-in-embedded-dashboard-programatically).