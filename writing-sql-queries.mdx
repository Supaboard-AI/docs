...
Zero values can be meaningful. Do **not remove rows simply because a value is zero**.
...
Yes. You can keep parts of your query unaffected by dashboard filters — just add a `$`. Read [Partial Filters](https://docs.supaboard.ai/ignore-filtering-for-a-given-table) for more info.